...
export default function QuickAdd() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<{ type: 'category' | 'website', id: string } | null>(null);
  ...
  const handleImportConfig = async () => {
    try {
      ...
      const formattedCategories = data.categories.map((cat: { name: string; urls: { url: string; title: string; account?: string; password?: string; visitedAt?: string }[] }) => ({
        id: Date.now().toString() + Math.random().toString(36).substr(2, 9),
        name: cat.name,
        websites: cat.urls.map((url: { url: string; title: string; account?: string; password?: string; visitedAt?: string }) => ({
          id: Date.now().toString() + Math.random().toString(36).substr(2, 9),
          url: url.url,
          title: url.title,
  ...
  const handleAddWebsite = async (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && newUrl.trim() && activeCategory) {
      ...
      try {
        ...
        const updatedCategory = updatedCategories.find(cat => cat.id === activeCategory);
        if (updatedCategory) {
  ...
  // 修改网址点击处理函数
  const handleWebsiteClick = (categoryId: string, websiteId: string, username: string) => {
          ...
          <div className="w-full">
            ...
            <div className="flex gap-4 relative">
              ...
              <div className="w-[1000px]">
                {activeCategory && categories.find(cat => cat.id === activeCategory)?.websites.map(website => (
                  <div 
                    key={website.id} 
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 mb-2 w-[400px]">
                      <a 
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleWebsiteClick(activeCategory, website.id, website.username);
                        }}
                      >
                    ...
                    {/* URL 区域 */}
                    <div className="text-sm text-gray-500 pl-7 truncate" title={website.url}>
                      {website.url}
                    </div>
                  </div>
                ))}
              ...
              <div className="w-[400px]">
                <div 
                  className={`credentials-container ${activeWebsite ? 'active' : ''}`}
                >
                  <div className={`p-4 border rounded transition-colors duration-300 active-credentials ${
                    activeWebsite ? 'border-blue-500 shadow-lg' : 'border-gray-200'
                  }`}>
